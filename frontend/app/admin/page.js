...
//     try {
//       const result = await apiClient.getUrls(pwd);
//       if (result.success) {
...
//     try {
//       const result = await apiClient.adminAuth(password);
//       if (result.success) {
//         // Save session
...
//       const result = await apiClient.getUrls(sessionPassword);
//       if (result.success) {
//         setUrls(result.data.urls);
//         setStats(result.data.stats);
//         setError('');
  ...
  const fetchUrls = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.error === "Unauthorized") logout();
  ...
  const deleteUrl = async (shortCode) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.error === "Unauthorized") logout();
        else setError(result.error || "Failed to delete URL");
      }
    } catch (err) {
        ...
        <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
          ...
          <div className="space-y-4">
            <input
              ...
              placeholder="Enter admin password"
              className="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              onKeyDown={(e) => e.key === "Enter" && !authLoading && authenticate()}
              disabled={authLoading}
            />
            <button
              onClick={authenticate}
