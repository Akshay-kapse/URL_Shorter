...
//       if (result.success) {
//         await fetchUserUrls(); // Refresh data
//       } else {
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      fetchUserUrls(storedUser.email);
    }
  }, []);
  ...
  const fetchUserUrls = async (email) => {
    ...
    try {
      const response = await fetch(`/api/urls?email=${email}`);
      const result = await response.json();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard title="Total URLs" value={stats.totalUrls} icon="🔗" color="text-blue-600" />
          <StatCard title="Total Visits" value={stats.totalVisits} icon="👁️" color="text-green-600" />
          <StatCard title="Recent URLs (24h)" value={stats.recentUrls} icon="⚡" color="text-purple-600" />
        </div>
